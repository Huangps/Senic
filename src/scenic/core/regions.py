...
from scenic.core.distributions import (Samplable, RejectionException, needsSampling,smt_add, smt_subtract, smt_multiply,
                                        smt_divide, smt_and, smt_equal, smt_assert, vector_operation_smt,writeSMTtoFile,findVariableName,
                                       smt_lessThan,smt_or,cacheVarName, checkAndEncodeSMT, smt_lessThanEq,distributionMethod)
...
def encodePolygonalRegion_SMT(smt_file_path, cached_variables, triangles, smt_var, debug=False):
	...
	for triangle in triangle_list:
		...
		if debug:
			plt.plot(*triangle.exterior.xy, color='g')  #绘制三角形
...
class PolygonalRegion(Region):
	...
	def encodeToSMT(self, smt_file_path, cached_variables, smt_var=None, debug=False):
		...
		# polygon_triangles = [
		# 	triangle[0]  # 提取三角形对象
		# 	for triangle in self.trianglesAndBounds
		# 	# 筛选条件: a 在 x 方向边界内，b 在 y 方向边界内
		# 	if (
		# 			triangle[1][0] < a < triangle[1][2]  # min_x < a < max_x
		# 			and triangle[1][1] < b < triangle[1][3]   # min_y < b < max_y
		# 	)
		# ]

		# # 获取第一个三角形及其边界框
		# first_tri, first_bounds = self.trianglesAndBounds[0]
		#
		# # 提取顶点坐标（假设三角形是 shapely.Polygon）
		# vertices = list(first_tri.exterior.coords)[:3]  # 取前三个顶点（闭合多边形会有重复点）
		#
		# # 打印结果
		# print("顶点坐标:")
		# for x, y in vertices:
