...
nusc = NuScenes(
    ...
    dataroot='D:/Scenic-main/nuscenesData/v1.0-Full-dataset/v1.0-trainval_meta/',
    verbose=False
)
...
def get_ego_from_fig(filename: str):
    ...
    current_fig_sample_data = next((sd for sd in nusc.sample_data
                        if filename in sd['filename']), None)
    if not current_fig_sample_data:
        raise FileNotFoundError(f"未找到文件 {filename}")
    ego_pose = nusc.get('ego_pose', current_fig_sample_data['ego_pose_token'])
...
def get_scene_location_from_fig(filename: str):
    ...
    sample = nusc.get('sample', current_fig_sample_data['sample_token'])
...
def render_annotation(filename: str, index: int, out_path: str):
    ...
    nusc.render_annotation(my_annotation_metadata['token'], extra_info=True, out_path=out_path)
...
def render_fig(filename: str):
    ...
    if not current_fig_sample_data:
        raise FileNotFoundError(f"未找到文件 {filename}")
    nusc.render_sample_data(current_fig_sample_data['token']) # 渲染当前图片
...
def main():
    ...
    # 示例图片文件名
    #image_file = 'n008-2018-08-01-15-16-36-0400__CAM_FRONT__1533151072912404'

    image_file ='n008-2018-08-01-15-16-36-0400__CAM_FRONT__1533151591862404'
    #image_file = 'n008-2018-07-27-12-07-38-0400__CAM_FRONT__1532707814912404'
    render_fig(image_file)
    #获取ego信息
    ...
    for i, obj in enumerate(object_data):
        if obj['type'] == 'human.pedestrian.adult' and is_in_view(ego_data['ego_position'], ego_data['ego_rotation'], obj['position']):
            print(f"目标 {i}:")
            print(f"  类型: {obj['type']}")
